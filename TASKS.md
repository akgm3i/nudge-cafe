...
- [ ] **タスク: 【サイクル基盤】「体験→応用」サイクルの基本システム実装**
  - 内容: 「体験」フェーズと「応用」フェーズの状態を管理し、一連のゲームサイクルを進行させるための基本的なロジックとUIコンポーネントを実装する。
  - 参照: `docs/dev/system.md`, `docs/dev/uiux.md`
  - 完了要件:
    - [ ] チャプターの進行状況（体験フェーズか応用フェーズか）を管理できる。
    - [ ] 各フェーズに応じたUI（NPCとの対話、メニュー開発など）を切り替えて表示できる。
  - 進捗: 未着手。
...
- [ ] **タスク: 【記録】新しい発見ノートのUIとロジック実装**
  - 内容: 新しい設計に基づいた「発見ノート」のUIと記録ロジックを実装する。
  - 参照: `docs/dev/system.md` (セクション2), `docs/dev/data-structure.md`
  - 完了要件:
    - [ ] 営業終了画面から「発見ノート」を開ける。
    - [ ] 「体験したこと」「そこからの気づき」など、新しい記録項目をUIに反映する。
...
- [ ] **タスク: 【シナリオ】第一部の章（おとり効果、アンカリング効果など）を実装**
  - 内容: `game-scenario.md`に基づき、第一部の各章の「体験→応用」サイクルを、Phase2で構築したシステム上で最後までプレイできるように実装する。
  - 参照: `docs/dev/game-scenario.md`
  - 完了要件:
    - [ ] 序章のチュートリアルがプレイできる。
    - [ ] 第一部の各チャプターが、「体験」から「記録」まで一気通貫で進行する。
  - 進捗: 未着手。
